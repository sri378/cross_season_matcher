...
def draw_card(db_path, snow_path, matches, inliers, ratio, score, out_path):
    ...
    cv2.putText(canvas, "SPRING/REFERENCE DATABASE", (15, h + 30),

                cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0,0,255), 2)

    cv2.putText(canvas, "WINTER/SNOW QUERY", (db.shape[1] + 55, h + 30),

                cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0,0,255), 2)



    cv2.putText(canvas, f"matches={matches} | inliers={inliers} | ratio={ratio:.3f} | score={score:.1f}",

                (15, h + 65), cv2.FONT_HERSHEY_SIMPLEX, 0.58, (0,0,0), 1)



    cv2.putText(canvas, f"DB: {db_path.parent.parent.name}/{db_path.parent.name}/{db_path.name[:45]}",

                (15, h + 92), cv2.FONT_HERSHEY_SIMPLEX, 0.42, (0,0,0), 1)

    cv2.putText(canvas, f"SNOW: {snow_path.parent.parent.name}/{snow_path.parent.name}/{snow_path.name[:45]}",

                (db.shape[1] + 55, h + 92), cv2.FONT_HERSHEY_SIMPLEX, 0.42, (0,0,0), 1)
    ...
    for pair_idx, snow_path in enumerate(snow_paths, start=1):
        ...
        for i, db_path in enumerate(db_paths, start=1):
            ...
            matches, inliers, ratio, score = match_pair(
                ...
                xfeat, db_path, snow_path, device,

                args.max_side, args.top_k, args.ransac_thresh
        ...
        for rank, r in enumerate(rows[:5], start=1):
            ...
            card_path = CARDS / f"snow_{SELECTED_IDS[pair_idx-1]}_rank_{rank:02d}_ratio_{r['ratio']:.2f}.jpg"
