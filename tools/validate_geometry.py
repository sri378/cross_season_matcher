...
# Landmark-based geometry validation
def main():
        ...
        rows.append(
            {
                ...
                "manual_points": len(ann),
                "manual_inliers": (
                    int(manual_mask.sum()) if manual_mask is not None else None
                ),
    ...
    summary = {
        ...
        "mean_four_corner_mace_px": (
            float(ok["four_corner_mace_px"].mean()) if len(ok) else None
        ),
        "max_four_corner_mace_px": (
            float(ok["four_corner_mace_px"].max()) if len(ok) else None
        ),
        "all_pairs_under_5px_mace": (
            bool(ok["passes_5px_mace"].all()) if len(ok) else False
        ),
        "mean_manual_reprojection_error_px": (
            float(ok["manual_reproj_mean_px"].mean()) if len(ok) else None
        ),
    }
